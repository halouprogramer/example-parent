package com.lvlvstart.ftp.demo.util;
...
public class FtpContinueClient
{
    ...
    public UploadStatus upload(String local, String remote)
        ...
        // 检查远程是否存在文件
        FTPFile[] files = ftpClient.listFiles(new String(remoteFileName.getBytes(CHARSET), "iso-8859-1"));
        if (files.length == 1)
        {
    ...
    public UploadStatus CreateDirecroty(String remote, FTPClient ftpClient)
        ...
        if (!directory.equalsIgnoreCase("/")
                && !ftpClient.changeWorkingDirectory(new String(directory.getBytes(CHARSET), "iso-8859-1")))
        {
            ...
            while (true)
            {
                String subDirectory = new String(remote.substring(start, end).getBytes(CHARSET), "iso-8859-1");
    ...
    public UploadStatus uploadFile(String remoteFile, File localFile, FTPClient ftpClient, long remoteSize)
        ...
        OutputStream out = ftpClient.appendFileStream(new String(remoteFile.getBytes(CHARSET), "iso-8859-1"));
        // 断点续传
        if (remoteSize > 0)
        {
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            String directory = "bohai";
            myFtp.connect("192.168.114.134", 21, "duiu", "abc1234");
             myFtp.ftpClient.makeDirectory(directory);
             myFtp.ftpClient.changeWorkingDirectory(directory);
             System.out.println(myFtp.upload("D:\\重庆没有计算得保单号.xls", "/重庆没有计算得保单号1.xls"));
            myFtp.disconnect();
        }
        catch (Exception e)
        {
